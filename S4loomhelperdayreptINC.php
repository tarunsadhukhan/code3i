...
$sqa="SELECT EBNO,A.WRK_NAME,TRAN_DATE,EFF,GEFF,0 CNT,NODAYS,EFF_INC,TOT_RATE FROM
(
SELECT EB_NO EBNO,TRAN_DATE,WRK_NAME,EFF,0 CNT,0 EFF_INC , NVL(TOT_RATE,0) TOT_RATE FROM ( 
SELECT TRAN_DATE,EB_NO,WRK_NAME,ROUND(SUM(QUANTITY)/SUM(STDPROD)*100,0) EFF,COUNT(*) DAYS FROM ( 
SELECT TRAN_DATE,A.SPELL,LINE_NO,QUANTITY,STDPROD,ATTANDANCE_DATE,EB_NO,B.SPELL,MECH_CODE,WRK_NAME FROM ( 
SELECT TRAN_DATE,SPELL,LINE_NO,SUM(QUANTITY) QUANTITY,SUM(STDPROD) STDPROD FROM ( 
SELECT TRAN_DATE,A.LOOM_NO,SPELL,QUANTITY,STDPROD,LINE_NO FROM DAILY_LOOM_DATA A,LOOM_MASTER B 
WHERE TRAN_DATE>=TO_DATE('$dt1','yyyy/mm/dd') AND TRAN_DATE<=TO_DATE('$dt2','yyyy/mm/dd')
AND A.LOOM_NO=B.LOOM_NO AND QUANTITY>0 AND LOOM_TYPE='G' ) GROUP BY TRAN_DATE,SPELL,LINE_NO 
 ) A, ( SELECT A.ATTANDANCE_DATE,B.EB_NO,A.SPELL,MECH_CODE,A.OCCU_ID,WRK_NAME FROM DAILYEBMCATT A,DAILY_ATTENDANCE B 
 WHERE A.ATTANDANCE_DATE>=TO_DATE('$dt1','yyyy/mm/dd') AND A.ATTANDANCE_DATE<=TO_DATE('$dt2','yyyy/mm/dd')
 AND A.OCCU_ID=79 AND A.ATT_TRAN_ID=B.ATT_TRAN_ID ) B WHERE A.TRAN_DATE=B.ATTANDANCE_DATE AND RTRIM(A.SPELL)=RTRIM(B.SPELL) AND A.LINE_NO=B.MECH_CODE ) 
 GROUP BY TRAN_DATE,EB_NO,WRK_NAME ) A,INCENTIVE_TABLE B WHERE A.EFF=B.EFF_INC(+) 
 ) A,
 (
SELECT EB_NO,WRK_NAME,ROUND(SUM(QUANTITY)/SUM(STDPROD)*100,0) GEFF,COUNT(*) NODAYS  FROM ( 
SELECT EB_NO,WRK_NAME,TRAN_DATE,SUM(QUANTITY) QUANTITY,SUM(STDPROD) STDPROD,ROUND(SUM(QUANTITY)/SUM(STDPROD)*100,0) EFF,COUNT(*) NODAYS  FROM ( 
SELECT TRAN_DATE,A.SPELL,LINE_NO,QUANTITY,STDPROD,ATTANDANCE_DATE,EB_NO,B.SPELL,MECH_CODE,WRK_NAME FROM ( 
SELECT TRAN_DATE,SPELL,LINE_NO,SUM(QUANTITY) QUANTITY,SUM(STDPROD) STDPROD FROM ( 
SELECT TRAN_DATE,A.LOOM_NO,SPELL,QUANTITY,STDPROD,LINE_NO FROM DAILY_LOOM_DATA A,LOOM_MASTER B 
WHERE TRAN_DATE>=TO_DATE('$dt1','yyyy/mm/dd') AND TRAN_DATE<=TO_DATE('$dt2','yyyy/mm/dd')
AND A.LOOM_NO=B.LOOM_NO AND QUANTITY>0 AND LOOM_TYPE='G' ) GROUP BY TRAN_DATE,SPELL,LINE_NO 
 ...
 WHERE A.ATTANDANCE_DATE>=TO_DATE('$dt1','yyyy/mm/dd') AND A.ATTANDANCE_DATE<=TO_DATE('$dt2','yyyy/mm/dd')
 AND A.OCCU_ID=79 AND A.ATT_TRAN_ID=B.ATT_TRAN_ID ) B WHERE A.TRAN_DATE=B.ATTANDANCE_DATE AND RTRIM(A.SPELL)=RTRIM(B.SPELL) AND A.LINE_NO=B.MECH_CODE ) 
 GROUP BY EB_NO,WRK_NAME ,TRAN_DATE 
 ) GROUP BY EB_NO,WRK_NAME  
 ) B WHERE A.EBNO=B.EB_NO ORDER BY EBNO,TRAN_DATE";
...
<div
   style="
      ...
      z-index: 3;
	  visibility: show;">
<td font size=45><?php echo "<br>" ." S4 Helpers Incentive Report From Dated :".$date->format('d-m-Y')." To ".$date1->format('d-m-Y') .$yarn."</br>"; ?></td>
</div>
