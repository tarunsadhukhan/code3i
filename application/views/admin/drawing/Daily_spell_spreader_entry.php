        ...
        <div class="form-group col-md-2" style="margin-left: 20px;" >
           ...
           <div class="form-group col-md-1" style="margin-left: 0px;">
            ...
            <div class="form-group col-md-1" style="margin-left: 0px;">
              ...
              <button name="submit" id="updatesprddoff" style="height: 40px;" type="submit" class="form-control btn btn-primary">Delete</button>
            </div>
            <div class="form-group col-md-1" style="margin-left: 0px;">
            ...
            <div class="form-group col-md-1" style="margin-left: 0px;">
						  <label for="purchaseDetailsPurchaseDate">Print Data<span class="text-center"></span></label>
              <button name="submit" id="printsprddbfdata" style="height: 40px;" type="submit" class="form-control btn btn-primary">Print</button>
            </div>
...
//start save
      ...
      $("#updatesprddoff").click(function(event){
          event.preventDefault(); 
          var companyId=$('#companyId').val();
          var windingDate= $('#windingDate').val();
          var recordId=$('#record_id').val();
          $.ajax({
    url: "<?php echo base_url('admin/Daily_spell_drawing_entry/updatesprddoff_data'); ?>",
            type: "POST",
...
$(document).keydown(function (event) {
        if (event.keyCode === 46 && selectedRecordId) { // 46 = Delete key
            event.preventDefault();
            confirmDelete(selectedRecordId);
        }
    });

    // Function to show confirmation and delete record
...
//               $sft='A'; 
               $('#shiftName').val(rowData[2]);
                $("#mc_no1").val(rowData[14]);
                $("#mc_no1").trigger('change');
                $("#cmeter").val(rowData[5]);
                $("#opmeter").val(rowData[6]);
                $("#clmeter1").val(rowData[7]);
                $("#dfmeter").val(rowData[8]);
                $("#eff").val(rowData[9]);
                $("#actroll").val(rowData[10]);
                $("#remarks").val(rowData[13]);
          ...
          $("#savewnddoff").attr('disabled',true);
          $("#updatesprddoff").attr('disabled',false);
...
$("#printsprddbfdata").click(function(event){
              event.preventDefault(); 
        //      alert('aabb');
	          var payrollstartdate= $('#windingDate').val();
            var companyId=$('#companyId').val();
      
            var url = '<?php echo site_url("admin/Daily_spell_drawing_entry/printdbfdata"); ?>' +
                      '?payrollstartdate=' + payrollstartdate +
                      '&companyId=' + companyId 
                      ;
//                      alert(url);
