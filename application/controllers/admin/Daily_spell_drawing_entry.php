...
//nomc=2
	...
	echo json_encode($response);
		...
		public function updatesprddoff_data() {
			$recordId = $this->input->post('recordId');
			$sql="delete from EMPMILL12.daily_drawing_transaction where drg_tran_id";
			$data = array(
				'recordId' =>$recordId,
			)	;			
		    $this->db->where('drg_tran_id', $recordId);
			$delete = $this->db->delete('EMPMILL12.daily_drawing_transaction');
	...
	//		var_dump($records);
		 ...
		 public function exportdbfdata() {
			...
			public function mpdfprintdbfdata() {
				$sdate = $this->input->post('payrollstartdate');
				$compid = $this->input->post('companyId');
				$sdate = $this->input->get('payrollstartdate');
				$compid = $this->input->get('companyId');
				...
				$records=$this->daily_spell_drawing_entry_model->getsprdspelldata($date , $compid);
				$html = '
				<style>
					...
					.date { text-align: right; font-size: 14px; font-weight: bold; margin-bottom: 10px; }
				</style>
				  
				<div class="header">THE EMPIRE JUTE CO LTD</div>
				<div class="date">Date: .<?php echo $sdate; ?>.</div>
				...
				<table>
					<tr>
						...
						<th>Meter Reading</th>
						<th>Difference</th>
						<th>Roll As Per Reading</th>
						<th>Actual Roll</th>
						<th>EB NO</th>
					</tr>';
				
				$data = [
					[1, "6 A.M TO 11 A.M", "820645", "13593", "84", "33", "Receiver: 6940"],
					[2, "6 A.M TO 11 A.M", "626732", "3750", "13", "12", "Feeder: 18840"],
					[3, "11 A.M TO 2 P.M", "839368", "1090", "40", "40", "Receiver: 19218"],
					[4, "2 P.M TO 5 P.M", "54572", "15321", "RTF Saw", "Feeder: 19082"],
				];
			...
			public function printdbfdata() {
				...
				$pdf->SetMargins(10, 10, 15);  // Left = 15mm, Top = 10mm, Right = 15mm
				...
				// Move to X=50, Y=30 and write text
				$pageWidth = $pdf->GetPageWidth();
				$pageHeight = $pdf->GetPageHeight();
				//$pdf->SetXY(50, 10);
				$pdf->Cell(0, 10, 'The Empire Jute Co. Ltd', 0, 1,'C');
				$x=1;
				$y=10;
			
				// Move to X=50, Y=40 and write another line
				$pdf->SetXY(150, 10);
				//$pdf->Cell(0, 10, 'Dated '.$sdate, 0, 1);
				$y=$y+4;
				$x=12;
				 
				// Move to X=50, Y=50 and write a date
				//$sdate = date('Y-m-d');
				$pdf->SetXY($x, $y);
				$pdf->Cell(0, $y, 'Daily Spreader Meter Report for Dated: ' . $sdate, 0, 1,'C');
				$y=$y+6	;
				//$pdf->Line(10, $y, 200, $y);  // (x1, y1, x2, y2)
			//	$y=$y+6;	
// Table Headers
/* $pdf->SetY($y);  // Move down
$pdf->Cell(20, 10, 'Spreader', 1, 0, 'C');  // 1 = border, 0 = same line, 'C' = Center
$pdf->Cell(15, 10, 'Spell', 1, 0, 'C');
$pdf->Cell(25, 10, 'Opening', 1, 0, 'C');  
$pdf->Cell(25, 10, 'Closing', 1, 0, 'C');  
$pdf->Cell(25, 10, 'Difference', 1, 0, 'C');  
$pdf->Cell(25, 10, 'Roll As', 0, 'C');
$pdf->Cell(20, 10, 'Actual', 1, 0, 'C');  
$pdf->Cell(30, 10, 'Feeder / Receiver', 1, 1, 'C');   */
$y=$y+6;
$pdf->SetY($y); 

$pdf->Cell(20, 5, 'Spreader', 0, 0, 'C');  
$pdf->Cell(20, 5, 'Spell', 0, 0, 'C');
$pdf->Cell(25, 5, 'Opening', 0, 0, 'C');  
$pdf->Cell(25, 5, 'Closing', 0, 0, 'C');  
$pdf->Cell(25, 5, 'Difference', 0, 0, 'C');  
$pdf->Cell(20, 5, "Roll As", 0, 0, 'C'); 
$pdf->Cell(20, 5, 'Actual', 0, 0, 'C');  
$pdf->Cell(30, 5, 'Feeder ', 0, 1, 'C'); 
$y=$y+4;
$pdf->Cell(20, 5, 'No', 0, 0, 'C');  
$pdf->Cell(20, 5, '', 0, 0, 'C');
$pdf->Cell(25, 5, 'Meter', 0, 0, 'C');  
$pdf->Cell(25, 5, 'Meter', 0, 0, 'C');  
$pdf->Cell(25, 5, 'Meter', 0, 0, 'C');  
$pdf->Cell(20, 5, "Per Meter", 0, 0, 'C'); 
$pdf->Cell(20, 5, 'Roll', 0, 0, 'C');  
$pdf->Cell(30, 5, 'Receiver ', 0, 1, 'C');  
$y=$y+4;
...
$pdf->Cell(25, 5, '', 0, 0, 'C');  
$pdf->Cell(25, 5, '', 0, 0, 'C');  
$pdf->Cell(20, 5, "Reading", 0, 0, 'C'); 
$pdf->Cell(20, 5, '', 0, 0, 'C');  
$pdf->Cell(30, 5, ' ', 0, 1, 'C');  
$y=$y+4;
$pdf->SetY($y); 
$pdf->Rect(10, 25, 185, 17); // (x, y, width, height)
$x=30;
$pdf->Line($x, $y-13, $x, $y+17);  // (x1, y1, x2, y2)
$x=$x+20;
$pdf->Line($x, $y-13, $x, $y+17);  // (x1, y1, x2, y2)
$x=$x+25;
$pdf->Line($x, $y-13, $x, $y+17);  // (x1, y1, x2, y2)
...
foreach ($records as $row) {
	/* if ($spl<>$row["spell"]) {
		$pdf->SetY($y); 
		$pdf->Cell(20, 10, $calcrl, 1, 0, 'C'); 
		$pdf->Cell(20, 10, $acrl, 1, 1, 'C'); 
		$calcrl=0;
$acrl=0;
$spl=$row["spell"];
	} */
	$pdf->Cell(20, 10, $row["mech_code"], 1, 0, 'C');  
	$pdf->Cell(20, 10, $row["spell"], 1, 0, 'C');
	$pdf->Cell(25, 10, $row["open_meter"], 1, 0, 'C');  
	$pdf->Cell(25, 10,$row["close_meter"], 1, 0, 'C');  
	$pdf->Cell(25, 10, $row["diff_meter"], 1, 0, 'C');  
	$pdf->Cell(20, 10, $row["calcroll"], 1, 0, 'C'); 
	$pdf->Cell(20, 10, $row['actual_prod'], 1, 0, 'C');  
	$pdf->Cell(30, 10, $row['eb_no_combined'], 1, 1, 'C');  
	$y=$y+10;
	$acrl=$acrl+$row['actual_prod'];
	$calcrl=$calcrl+$row["calcroll"];
	if ($pdf->GetY() > 250) { // If Y position is greater than 250mm
...
// Draw a rectangle border around the table
//$pdf->Rect(10, 25, 160, 30); // (x, y, width, height)



/* $colWidths = [30, 20, 25, 25, 25, 30, 20, 30];
$headers = [
    "Spreader No", "Spell", "Opening", "Closing", 
    "Difference", "Roll As Per\nMeter Reading", "Actual Roll", "Feeder / Receiver"
];
foreach ($headers as $key => $header) {
    $pdf->MultiCell($colWidths[$key], 10, $header, 1, 'C');
    $pdf->SetXY($pdf->GetX() + $colWidths[$key], $pdf->GetY() - 10);  // Adjust position
}
$pdf->Ln(); 

$data = [
    [12001, "A1", 25896, 26896, 1000, 60, 59, "12589 / 25896"]
];
...
foreach ($data as $row) {
    foreach ($row as $key => $cell) {
        if ($key == 5 || $key == 7) { 
            // Wrap text for "Roll As Per Meter Reading" and "Feeder / Receiver"
            $pdf->MultiCell($colWidths[$key], 10, $cell, 1, 'C');
            $pdf->SetXY($pdf->GetX() + $colWidths[$key], $pdf->GetY() - 10);
        } else {
            $pdf->Cell($colWidths[$key], 10, $cell, 1, 0, 'C');
        }
    }
    $pdf->Ln();
}
