...
class Daily_Attendance_Entry extends CI_Controller {
...
//		$this->load->view('admin/winding_doff/winding_doff_data',$data,$dataq);
		$this->load->view('admin/attendance/Daily_Attendance_Entry', $data_to_pass);	
	}
...
//var_dump ($response);
    ...
    public function getdesigdata() {
        $category_id = $this->input->post('mcno1');

        if ($category_id) {
            // Fetch subcategories from the model based on category_id
            $subcategories = $this->daily_spell_drawing_entry_model->getdesigdata($category_id);

            // Send the subcategories as JSON response
            echo json_encode($subcategories);
        } else {
            echo json_encode([]); // Return an empty array if no category is selected
        }
    }
    public function getmcnodata() {
        $category_id = $this->input->post('mcno1');
        $desig_id = $this->input->post('mcno2');



        if ($desig_id) {
            // Fetch subcategories from the model based on category_id
            $subcategories = $this->daily_spell_drawing_entry_model->getmcnodata($category_id,$desig_id);
	...
	public function saveatt_data() {
            ...
            $mcnos =$this->input->post('mcnos');
            $ebid1 = $this->input->post('ebid1');
            $ebno1 = $this->input->post('ebno1');
            $splhrs1 = $this->input->post('splhrs1');
            $whrs = $this->input->post('whrs');
            $idlehrs = $this->input->post('idlehrs');
            ...
            'spell_hours' => $splhrs1,
            'status_id' => 3,
            'worked_department_id' => $deptid,
            'worked_designation_id' => $desigid,
            'working_hours' => $whrs,
   
		);
	//	$this->db->insert('WINDING_SPELL_EB_PROD_QLTY', $data);
		$this->db->insert('daily_attendance', $data1);
 

        $sql="select max(daily_atten_id) maxid from daily_attendance";
        $query = $this->db->query($sql);
        $row1 = $query->row();
		$cnt = $query->num_rows();
        $maxid=0;
        ...
        foreach ($mcnos as $mc_no) {
            // Insert each selected mc_no into the database
            // Example:
            if ($mc_no>0) {
            $data = array(
                'attendace_date' => $windingDate,
                'company_id' => $companyId,
                'created_by' => 26586,
                'daily_atten_id' => $maxid,
                'eb_code' => $ebno1,
                'eb_id' =>$ebid1,
                'eb_no' => $ebno1,
                'is_active' => $active,
            ...
            $this->db->insert('daily_ebmc_attendance', $data);
        }
    }
...
//nomc=2
	...
	echo json_encode($response);
		...
		public function updateattndata() {
			$record_id = $this->input->post('record_id');
			 
	//		echo $recordId;
			// Load the database library
			$sql="update daily_ebmc_attendance set is_active=0 where daily_atten_id=".$record_id;
			$this->db->query($sql);
			$sql="update daily_attendance set is_active=0 where daily_atten_id=".$record_id;
			$this->db->query($sql);
	...
	//		echo $recordId;
			// Load the database library
			$sql="update daily_ebmc_attendance set is_active=0 where daily_atten_id=".$recordId;
			$this->db->query($sql);
			$sql="update daily_attendance set is_active=0 where daily_atten_id=".$recordId;
			$this->db->query($sql);
		    $response = array(
				'success' => true,
				'frameNo' => $recordId,
		...
		public function get_records() {
			...
			$records=$this->daily_spell_drawing_entry_model->getattndata($date, $shift, $compid,$deptid);
			$cnt=count($records);
            /*
            <th>Record ID</th>
            <th>Date</th>
 	...
 	*/
			...
			if ($cnt>0) {
				foreach ($records as $record) {
					$data[] = [
						$record['daily_atten_id'],         // Use array notation instead of object property
						$record['Date'],       // Use array notation instead of object property
						$record['Spell'],          // Use array notation instead of object property
						$record['Department'],   // Use array notation instead of object property
						$record['EB_No'],       // Use array notation instead of object property
						$record['Name'],       // Use array notation instead of object property
						$record['Designation'],      // Use array notation instead of object property
						$record['Working_Hours'],     // Use array notation instead of object property
						$record['idlehrs'],       // Use array notation instead of object property
						$record['MC_Nos'],       // Use array notation instead of object property
						$record['eb_id'],       // Use array notation instead of object property
						$record['worked_designation_id'],       // Use array notation instead of object property
						$record['worked_department_id'],       // Use array notation instead of object property
						$record['mcids'],       // Use array notation instead of object property
						$record['attendance_type'],       // Use array notation instead of object property
					];
				}
		}
			// Return the response
