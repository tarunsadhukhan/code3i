...
//            echo $records;
	...
	public function mcno1_checkdata() {
		$mcno1=$this->input->post('mcno1');
		$windingqcDate=$this->input->post('windingqcDate');
		$companyId = $this->input->post('companyId'); 
		$qcshiftName=$this->input->post('qcshiftName');
	 	
		$created_by=26577;
		$active=1;
		$cnt=0;
		$windingqcDate=substr($windingqcDate,6,4).'-'.substr($windingqcDate,3,2).'-'.substr($windingqcDate,0,2);
		$this->load->model('Winding_doff_Model');
		
		$records = $this->Winding_doff_Model->mcno1_checkdata($companyId,$windingqcDate,$qcshiftName,$mcno1); 	
		$cnt=count($records);
		if ($cnt==0) {
			$savedata=$cnt;
		}
